...
// TestHandleMovesItemAndBroadcasts verifies the successful move path.
func TestHandleMovesItemAndBroadcasts(t *testing.T) {
	...
	if len(*sent) != 2 || (*sent)[0].Header != outupdate.Header || (*sent)[1].Header != outheightmapupdate.Header {
		t.Fatalf("expected floor item update then height map update packets, got %#v", *sent)
	}
}

// TestHandleBroadcastsUpdateToOtherOccupants verifies both connections in a room observe a move.
func TestHandleBroadcastsUpdateToOtherOccupants(t *testing.T) {
	handler, connections := handlerForTest(t)
	...
	if len(*actorSent) != 2 || (*actorSent)[0].Header != outupdate.Header || (*actorSent)[1].Header != outheightmapupdate.Header {
		t.Fatalf("expected actor to receive floor item update then height map update, got %#v", *actorSent)
	}
	if len(*bystanderSent) != 2 || (*bystanderSent)[0].Header != outupdate.Header || (*bystanderSent)[1].Header != outheightmapupdate.Header {
		t.Fatalf("expected bystander to receive floor item update then height map update, got %#v", *bystanderSent)
	}
}

// TestHandleMovingOccupiedChairStandsOccupantUp verifies moving a chair to a new tile stands its
// occupant up and syncs the resulting status to every room connection.
func TestHandleMovingOccupiedChairStandsOccupantUp(t *testing.T) {
	handler, connections := handlerForTest(t)
	actor, actorSent := registeredConnectionForTest(t, connections, "conn")
	room, _ := handler.Runtime.Find(9)
	...
	if len(units) != 1 || unitHasStatus(units[0].Statuses, worldunit.StatusSit) {
		t.Fatalf("expected occupant to stand up after the chair moved, got %#v", units)
	}
	if len(*actorSent) != 3 || (*actorSent)[0].Header != outupdate.Header || (*actorSent)[1].Header != outstatus.Header || (*actorSent)[2].Header != outheightmapupdate.Header {
		t.Fatalf("expected floor update, unit status, then height map update packets, got %#v", *actorSent)
	}
}

// TestHandleRotatingOccupiedChairReorientsOccupant verifies rotating a chair in place turns its
// occupant along with it instead of standing them up or rejecting the rotation.
func TestHandleRotatingOccupiedChairReorientsOccupant(t *testing.T) {
	handler, connections := handlerForTest(t)
	actor, actorSent := registeredConnectionForTest(t, connections, "conn")
	room, _ := handler.Runtime.Find(9)
	settleUnitOnChair(t, room, 7, 1)
	...
	err := handler.Handle(context.Background(), command.Envelope[Command]{
		Command: Command{Handler: actor, ItemID: 1, X: 2, Y: 0, Rotation: int(worldunit.RotationEast)},
	})
	if err != nil {
		t.Fatalf("handle rotate: %v", err)
	}

	units := room.Units()
	if len(units) != 1 || !unitHasStatus(units[0].Statuses, worldunit.StatusSit) {
		t.Fatalf("expected occupant to remain seated after rotating in place, got %#v", units)
	}
	if units[0].BodyRotation != worldunit.RotationEast {
		t.Fatalf("expected occupant to rotate with the chair, got %#v", units[0])
	}
	if len(*actorSent) != 3 || (*actorSent)[0].Header != outupdate.Header || (*actorSent)[1].Header != outstatus.Header || (*actorSent)[2].Header != outheightmapupdate.Header {
		t.Fatalf("expected floor update, unit status, then height map update packets, got %#v", *actorSent)
	}
...
// TestHandleIgnoresSoftResolveErrors verifies gameplay misses stay silent and send a bubble alert.
func TestHandleIgnoresSoftResolveErrors(t *testing.T) {
	handler, connections := handlerForTest(t)
	connection, sent := registeredConnectionForTest(t, connections, "conn")
	handler.Furniture = &fakeManager{item: placedItemForTest(), itemFound: true}

	err := handler.Handle(context.Background(), command.Envelope[Command]{
		Command: Command{Handler: connection, ItemID: 1, X: 3, Y: 0, Rotation: 0},
	})
	if err != nil {
		t.Fatalf("expected soft resolve error to stay silent, got %v", err)
	}
...
// TestHandlePropagatesFindItemHardErrors verifies unexpected lookup errors surface.
func TestHandlePropagatesFindItemHardErrors(t *testing.T) {
	handler, _ := handlerForTest(t)
...
// TestHandlePropagatesMoveHardErrors verifies unexpected persistence errors surface.
func TestHandlePropagatesMoveHardErrors(t *testing.T) {
	handler, _ := handlerForTest(t)
	expected := errors.New("move failed")
	handler.Furniture = &fakeManager{
		definition: chairDefinitionForTest(), definitionFound: true,
		item: placedItemForTest(), itemFound: true, moveErr: expected,
	}
...
// TestHandleMovesAndPublishesWithLogger verifies event publication and logging on success.
func TestHandleMovesAndPublishesWithLogger(t *testing.T) {
	handler, connections := handlerForTest(t)
	...
	if len(publisher.events) != 1 || publisher.events[0].Name != movedevent.Name {
		t.Fatalf("expected moved event published, got %#v", publisher.events)
	}
}

// TestHandleLogsRejectionWithLogger verifies rejected moves log without error when a logger is set.
...
// handlerForTest creates a move command handler with a loaded room and joined player.
...
// chairDefinitionForTest returns a placeable sit definition fixture with a single-tile seat slot,
// matching a real floor-height chair so moving or rotating it exercises real slot reconciliation.
func chairDefinitionForTest() furnituremodel.Definition {
	return furnituremodel.Definition{
		...
		Width: 1, Length: 1, StackHeight: 0, AllowSit: true,
		Metadata: []byte(`{"slots":[{"dx":0,"dy":0,"status":"sit","body_rotation":0}]}`),
	}
}

// placedItemForTest returns an already placed item fixture owned by the demo player.
func placedItemForTest() furnituremodel.Item {
	roomID, x, y, z := int64(9), 0, 0, 0.0
...
func (manager *fakeManager) Move(_ context.Context, params furnitureservice.MoveParams) (furnituremodel.Item, error) {
	if manager.moveErr != nil {
		return furnituremodel.Item{}, manager.moveErr
	}

	roomID, x, y, z := int64(9), params.Placement.X, params.Placement.Y, params.Placement.Z

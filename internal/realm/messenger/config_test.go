...
func TestNormalizeUsesDefaults(t *testing.T) {
	...
	if config.MaxFriends != 200 || config.MaxFriendsClub != 500 || config.SearchCacheTTL != 30*time.Second || config.ChatThrottle != 750*time.Millisecond {
...
func TestLoadConfigUsesEnvironment(t *testing.T) {
	...
	if err != nil || config.MaxFriends != 300 || config.SearchCacheTTL != 45*time.Second || !config.ChatLogEnabled {

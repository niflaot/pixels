// Package repository persists messenger relationships and requests.
package repository
...
type Store interface {
	...
	ListFriends(context.Context, int64) ([]messengermodel.Friendship, error)
	// ListFollowers returns viewer-specific rows that point at one friend.
	ListFollowers(context.Context, int64) ([]messengermodel.Friendship, error)
	...
	ListIncomingRequests(context.Context, int64) ([]messengermodel.Request, error)
	// ListOutgoingRequests returns requests sent by a player.
	ListOutgoingRequests(context.Context, int64) ([]messengermodel.Request, error)
	...
	SetRelation(context.Context, int64, int64, messengermodel.Relation) (bool, error)
	// RelationshipSummaries returns public non-empty relationship categories.
	RelationshipSummaries(context.Context, int64) ([]messengermodel.RelationshipSummary, error)
	// ListIgnored returns one player's directional ignore list.
	ListIgnored(context.Context, int64) ([]messengermodel.IgnoredPlayer, error)
	...
	SearchByPrefix(context.Context, string, int) ([]messengermodel.SearchResult, error)

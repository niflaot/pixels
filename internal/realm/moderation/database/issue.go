...
func (repository *Repository) CreateIssue(ctx context.Context, params moderationrecord.ReportParams) (moderationrecord.Issue, error) {
	...
	err := postgres.WithinScope(ctx, repository.pool, func(txCtx context.Context) error {
		row := repository.executor(txCtx).QueryRow(txCtx, `insert into moderation_issues(reporter_player_id,reported_player_id,room_id,topic_id,kind,message) values($1,$2,$3,$4,$5,$6) returning id,reporter_player_id,reported_player_id,room_id,topic_id,kind,message,state,resolution,picked_by_player_id,picked_at,closed_by_player_id,closed_at,created_at`, params.ReporterPlayerID, params.ReportedPlayerID, params.RoomID, params.TopicID, params.Kind, params.Message)

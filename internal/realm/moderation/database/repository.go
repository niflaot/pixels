...
func scanStaffIssue(row rowScanner) (moderationrecord.Issue, error) {
	var value moderationrecord.Issue
	err := row.Scan(&value.ID, &value.ReporterPlayerID, &value.ReportedPlayerID, &value.RoomID, &value.TopicID, &value.Kind, &value.Message, &value.State, &value.Resolution, &value.PickedByPlayerID, &value.PickedAt, &value.ClosedByPlayerID, &value.ClosedAt, &value.CreatedAt, &value.ReporterName, &value.ReportedName, &value.PickerName)
	return value, err
}

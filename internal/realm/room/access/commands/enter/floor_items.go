...
func (handler Handler) sendFloorItems(ctx context.Context, connection netconn.Context, room roommodel.Room, active *roomlive.Room) error {
	...
	owners, records := projection.FloorItemsVisible(items, definitions, names, senders, room.HideWired)
	if handler.Groups != nil {
		for index := range records {
			group, found := handler.Groups.Furniture(records[index].ID)
			if found {
				records[index].Data = stuffdata.StringArray([]string{records[index].ExtraData, strconv.FormatInt(group.GroupID, 10), group.BadgeCode, group.ColorAHex, group.ColorBHex})
			}
		}
	}

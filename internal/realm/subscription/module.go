...
func NewService(config Config, store record.Store, players playerservice.ClubWriter, livePlayers *playerlive.Registry, currencies currencyservice.Granter, furniture furnitureservice.DefinitionGranter, catalog *catalogservice.Service, events bus.Publisher) *core.Service {
	config = config.Normalize()
	return core.New(core.Options{PaydayInterval: config.PaydayInterval, KickbackPercentage: config.KickbackPercentage, PaydayCurrencyType: config.PaydayCurrencyType}, store, players, currencies, furniture, catalog, events, livePlayers)
}

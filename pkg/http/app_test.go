...
func testApp(t *testing.T, environment string) *fiber.App {
	...
	return New(zap.NewNop(), config, testInfo(), service, adapter, testRooms(), testRoomRuntime(), nil, testNavigator(), testCurrencyDependencies(registry, zap.NewNop()), testCatalogDependencies(registry, zap.NewNop()))
}

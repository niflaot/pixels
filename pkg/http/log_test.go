...
func testAppWithLogger(t *testing.T, environment string, log *zap.Logger) *fiber.App {
	...
	return New(log, config, testInfo(), service, adapter, testRooms(), testRoomRuntime(), nil, testNavigator(), testCurrencyDependencies(registry, log), testCatalogDependencies(registry, log))
}
